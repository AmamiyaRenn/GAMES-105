...
def part1(viewer, bvh_file_path):
    ...
    """
    joint_name, joint_parent, joint_offset = part1_calculate_T_pose(
        bvh_file_path)
...
def part2_one_pose(viewer, bvh_file_path):
    ...
    joint_positions, joint_orientations = part2_forward_kinematics(
        joint_name, joint_parent, joint_offset, motion_data, 0)
...
def part2_animation(viewer, bvh_file_path):
    ...
    class UpdateHandle:
        ...
        def update_func(self, viewer_):
            joint_positions, joint_orientations = part2_forward_kinematics(
                joint_name, joint_parent, joint_offset, motion_data,
                self.current_frame)
            viewer.show_pose(joint_name, joint_positions, joint_orientations)
...
def part3_retarget(viewer, T_pose_bvh_path, A_pose_bvh_path):
    ...
    joint_name, joint_parent, joint_offset = part1_calculate_T_pose(
        T_pose_bvh_path)
    # A-pose的动作数据
    retarget_motion_data = part3_retarget_func(
        T_pose_bvh_path, A_pose_bvh_path)
    ...
    class UpdateHandle:
        ...
        def update_func(self, viewer_):
            joint_positions, joint_orientations = part2_forward_kinematics(
                joint_name, joint_parent, joint_offset, retarget_motion_data,
                self.current_frame)
            viewer.show_pose(joint_name, joint_positions, joint_orientations)
            self.current_frame = (self.current_frame + 1) % frame_num
